...
The Play page is backed by an AWS Lambda deployed with SST, living in the `infra/` workspace (decoupled from the Next.js build). **When working inside `infra/`, read [`infra/CLAUDE.md`](infra/CLAUDE.md)** — it has the module map, request lifecycle, defense layers, and all Lambda/SST/Groq gotchas.
...
- **`infra/` is excluded** from the root `tsconfig.json`, from `eslint src/`, and from the root `npm test`. It has its own tsconfig/vitest/package.json — run its checks with `cd infra && npm test && npm run typecheck`. `groq-sdk` lives ONLY in `infra/packages/functions/package.json`, never in the root. **All Lambda/SST/Groq gotchas live in [`infra/CLAUDE.md`](infra/CLAUDE.md)** (streaming, `temperature: 0`, CORS method length, reserved-concurrency quota, signed-token replay, the `awslambda` global, push-deploys-both-targets).
- **Two files are duplicated across the Next↔Lambda boundary** to keep the build decoupled (the `infra/` side is canonical): `AgentEvent` (`infra/packages/shared/src/events.ts` → mirror `src/lib/agent-events.ts`, guarded by `tests/agent-stream.test.ts`) and the signed-token logic (`infra/packages/functions/src/token.ts` verifier → `src/lib/agent-token.ts` issuer, sharing `AGENT_SIGNING_SECRET`). Edit a canonical file and its mirror in the same change.
- **`AGENT_SIGNING_SECRET` must hold the same value on both sides** (Fly secret + SST Secret `AgentSigningSecret`) — a mismatch 403s every production agent request.
