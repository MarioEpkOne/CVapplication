...
- **Lambda streaming is enabled by `streaming: true` on `sst.aws.Function`, NOT `url.invokeMode`.** SST derives the Function URL `invokeMode` from `streaming` (`RESPONSE_STREAM` vs `BUFFERED`); `url.invokeMode` is not a valid property and is silently ignored. Without `streaming: true` the URL stays `BUFFERED` and only the handler's *first* `responseStream.write()` is delivered (fixed-`Content-Length` response) — the agent appears to emit one event then stop. Verify after deploy with `aws lambda get-function-url-config ... --query InvokeMode` (must be `RESPONSE_STREAM`).
- **Groq Llama 3.3 70B + tool calls require `temperature: 0`.** At the default temperature the model intermittently emits tool calls in a malformed `<function=name{...}>` text syntax, which Groq rejects with a 400 `tool_use_failed` (measured ~4/10 failures; 0/10 at `temperature: 0`). `agent.ts` sets `temperature: 0` and retries once on `tool_use_failed`. The catch-all maps any non-429 Groq error to "could not reach its model" — check CloudWatch / reproduce locally to see the real error.
